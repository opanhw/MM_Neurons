...
def main():
    ...
    if args.model_type == 'llava':
        ...
        for n in range(model.config.num_hidden_layers):
            ...
            for i in tqdm(range(bz_num)):
                if i == bz_num - 1:
                    ...
                    top_token = prob[i * args.bz:(i + 1) * args.bz, :].sort(descending=True, dim=1).indices[:, :args.top_tokens_num]
                top_word = []
        ...
        model = InstructBlipForConditionalGeneration.from_pretrained(args.model_path, low_cpu_mem_usage=True, device_map="auto")
