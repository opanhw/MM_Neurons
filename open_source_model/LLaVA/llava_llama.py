...
class LlavaLlamaForCausalLM(LlamaForCausalLM, LlavaMetaForCausalLM):
    ...
    def set_get_activations(self, start=None, end=None):
    ...
    def forward(
        ...
        if self.shuffle:
            idx = torch.cat([torch.tensor(range(52)), self.idx + 52, torch.tensor(range(52 + 256, inputs_embeds.shape[1]))], dim=0)
        ...
        if self.cal_activations:
            self.activations = [(self.act[n][0].detach()[0]).half().cpu() for n in
                                   range(self.config.num_hidden_layers)]
